...
export const calculate = (input: string) => {
  ...
  var iterate = () => {
    while (currentLetter !== letterEnd) {
      ...
      if (pointer === Pointer.start) {
        if (goLeft(dict, position0)) { position1.x --; pointer = Pointer.left }
        else if (goRight(dict, position0)) {position1.x ++; pointer = Pointer.right}
        else if (goUp(dict, position0)) {position1.y --; pointer = Pointer.up}
        else if (goDown(dict, position0)) {position1.y ++; pointer = Pointer.down}
      }
      else if (pointer === Pointer.right) {
        if (currentLetter === corner) {
          if (goUp(dict, position0)) {position1.y --; pointer = Pointer.up}
          else if (goDown(dict, position0)) {position1.y ++; pointer = Pointer.down}
          else return Error;
        }
